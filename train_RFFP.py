...
def calc_metrics(y_true, y_pred, mode=None, to_print=False):
    ...
    f1 = f1_score((test_preds[non_zeros] > 0), (test_truth[non_zeros] > 0), average='macro')
    ...
    return accuracy_score(binary_truth, binary_preds), mae, corr, f1
...
if __name__ == '__main__':
    ...
    result_recorder = ResultRecorder(os.path.join(opt.log_dir, opt.name, 'result.tsv'),
                                     total_cv=total_cv, corpus_name=opt.corpus_name)  # init result recoreder
    ...
    best_eval_acc, best_eval_uar, best_eval_macro_f1, best_eval_weighted_f1, best_eval_f1, best_eval_mae = 0, 0, 0, 0, 0, 0
    ...
    for epoch in range(opt.epoch_count,
        ...
        # eval val set
        acc, uar, macro_f1, weighted_f1, cm = eval(model, val_dataset)
        ...
        if opt.has_test and opt.verbose:
            acc, uar, macro_f1, weighted_f1, cm = eval(model, tst_dataset)
            logger.info('Tst result of epoch %d / %d acc %.4f uar %.4f macro_f1 %.4f weighted_f1 %.4f' % (
                epoch, opt.niter + opt.niter_decay, acc, uar, macro_f1, weighted_f1))
            logger.info('\n{}'.format(cm))
    ...
    if not opt.has_test:
        ...
        acc, uar, macro_f1, weighted_f1, cm = eval(model, tst_dataset, is_save=True, phase='test', epoch=best_eval_epoch)
