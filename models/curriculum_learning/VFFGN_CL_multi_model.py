...
from models.curriculum_learning.VFFGN_CL_single_model import VFFGNCLsingleModel
...
class VFFGNCLmultiModel(BaseModel):
    @staticmethod
    ...
    def load_pretrained_encoder(self, opt):
        pretrained_base_path = os.path.join(opt.checkpoints_dir, opt.name, 'single')
        print('Init parameter from {}'.format(pretrained_base_path))
        pretrained_path = os.path.join(pretrained_base_path, str(opt.cvNo))
        pretrained_config_path = os.path.join(pretrained_base_path, 'train_opt.conf')
        ...
        self.pretrained_encoder = VFFGNCLsingleModel(pretrained_config)
