...
class BaseModel(ABC):
    ...
    def calculate_and_save_similarity_matrix(self):
        """计算特征原型并保存相似矩阵"""
        pass
    ...
    def cuda(self):
        ...
        if not self.gpu_ids:
            raise ValueError("gpu_ids list is empty")

        available_gpus = torch.cuda.device_count()

        for device_id in self.gpu_ids:
            if not isinstance(device_id, int) or device_id < 0 or device_id >= available_gpus:
                raise ValueError(f"Invalid GPU ID: {device_id}")

        for name in self.model_names:
            net = getattr(self, 'net' + name)
            print(f"Moving {name} to cuda:{self.gpu_ids[0]}")  # 打印调试信息
